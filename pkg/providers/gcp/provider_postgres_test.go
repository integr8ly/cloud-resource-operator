...
func TestPostgresProvider_ReconcilePostgres(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "success creating finalizer",
			fields: fields{
				Client: func() client.Client {
					mc := moqClient.NewSigsClientMoqWithScheme(scheme)
					mc.CreateFunc = func(ctx context.Context, obj runtime.Object, opts ...client.CreateOption) error {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if statusMessage != tt.want {
				t.Errorf("ReconcilePostgres() statusMessage = %v, want %v", statusMessage, tt.want)
			}
		})
	}
	...
	tests := []struct {
		...
		},
		{
			...
			fields: fields{
				Client: func() client.Client {
					mc := moqClient.NewSigsClientMoqWithScheme(scheme)
					mc.DeleteFunc = func(ctx context.Context, obj apimachinery.Object, opts ...client.DeleteOption) error {
		...
		},
		{
			...
			fields: fields{
				Client: func() client.Client {
					mc := moqClient.NewSigsClientMoqWithScheme(scheme)
					mc.UpdateFunc = func(ctx context.Context, obj apimachinery.Object, opts ...client.UpdateOption) error {
						return fmt.Errorf("generic error")
					}
					return mc
				}(),
